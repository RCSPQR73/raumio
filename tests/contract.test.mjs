...
test('expanded market sample keeps normalized prices transparent',()=>{const sourceCount=objects.reduce((sum,o)=>sum+o.comparables.length,0);assert.ok(sourceCount>=14);const pair=objects.find(o=>o.id==='nightstand').comparables.find(c=>c.location==='Neuss');assert.equal(pair.price,35);assert.match(pair.similarity,/70 € für das Paar/);});
