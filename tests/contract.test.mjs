...
test('price totals count quantities and exclude unsupported categories',()=>{
 ...
 assert.equal(valued.length,10);
 assert.equal(total(objects).count,10);
 for(const o of valued){const ranged=o.comparables.filter(c=>c.useInRange!==false);assert.equal(o.min,Math.min(...ranged.map(c=>c.price)));assert.equal(o.max,Math.max(...ranged.map(c=>c.price)));}
 const expected=valued.filter(o=>o.includeInTotal!==false).reduce((a,o)=>({min:a.min+o.min*o.quantity,max:a.max+o.max*o.quantity}),{min:0,max:0});
...
test('all sources have bilingual explanations and unique listing links',()=>{for(const o of objects){const ids=new Set();for(const c of o.comparables){const u=new URL(c.url);assert.equal(u.hostname,'www.kleinanzeigen.de');assert.ok(u.pathname.startsWith('/s-anzeige/'));assert.ok(c.price>0);assert.ok(c.titleEn?.length>12);assert.ok(c.similarity.length>35);assert.ok(c.similarityEn?.length>35);assert.ok(!ids.has(c.url));ids.add(c.url);}}});
...
test('additional model groups have distinct mesh targets and cautious pricing',()=>{assert.equal(objects.length,12);for(const id of ['desk','curtain','vase','ceiling','cabinet','plant']){const o=objects.find(row=>row.id===id);assert.ok(o,`missing ${id}`);assert.ok(o.nodes.length>=3);assert.ok(o.query.length>0);}assert.equal(objects.find(o=>o.id==='desk').includeInTotal,false);assert.equal(objects.find(o=>o.id==='cabinet').min,null);assert.deepEqual(objects.find(o=>o.id==='unknown').nodes,[440,441,442]);assert.deepEqual(total(objects),{min:347,max:487,count:10});});
